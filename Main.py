...
sign="train"
basePath=""
new_cluster_path = "/user/ml/qiangyao_tmp/goods_cluster.result"
cluster_file = "./goods_cluster.result"
...
def addCluster():
    cluster_path = "%s/ctr_common_%s/cluster/"%(basePath,sign)
    os.system("hadoop fs -rmr %s" %(cluster_path))
    os.system("hadoop fs -mkdir %s" %(cluster_path))
    os.system("rm %s" %(cluster_file))
    os.system("hadoop fs -get %s %s" %(new_cluster_path , cluster_file))
    os.system("hadoop fs -put %s %s" %(cluster_file, cluster_path)) 
    os.system("hadoop fs -rmr %s/ctr_common_%s/user_goods_info_add_cluster"%(basePath,sign))
    tempOutput = "%s/ctr_common_%s/user_goods_info_add_cluster"%(basePath,sign)
    hour=None
    colLimit=4
    ...
    if(argvDict.has_key("hour")):
        hour=argvDict["hour"]
    job = HadoopJobProducer()
    job.setJobName("add_cluster_id")
    job.setMapstr("python AddClusterMapper.py")
    job.setReducestr("python AddClusterReducer.py")
    job.setJobPriority("NORMAL")
    job.addFile(["AddClusterMapper.py","AddClusterReducer.py","/hadoop/hive/lib/hive-exec-0.14.0.jar"])
    partName="*"
    if(argvDict.has_key("part")):
        partName=argvDict["part"]
    job.addInput("%s/ctr_common_%s/user_goods_info"%(basePath,sign),"",partName)
    job.addInput("%s/ctr_common_%s/cluster/"%(basePath,sign),"",partName)
...
def dataFilt():
    ...
    job.addInput("%s/ctr_common_%s/user_goods_info_add_cluster"%(basePath,sign),"",partName)
    job.setReduceNum(4847)
    #job.setReduceNum(0)
    job.setOutput(tempOutput)
    job.addOtherInfomation("-jobconf mapred.min.split.size=67108864")
